...
Wie StreamMusik intern funktioniert — für alle, die mehr wissen wollen als „Plugin installieren und nutzen".
...
1. **`TRACK_INFO_SCRIPT`** — fragt Music ab. Liefert `state`, `name`, `artist`, `album`, `duration`, `position`, `pid` (persistent ID) und schreibt das Cover in `/tmp/streammusik-artwork-<pid>.png` (idempotent, gecached per Persistent Track ID).
